...
struct linux_proc_info {
    ...
    unsigned long start_stack;      /* address of the start of the main process stack */
    ...
    unsigned long arg_start;        /* address above which program command line is placed */
    unsigned long arg_end;          /* address below which program command line is placed */
...
int read_proc_info(int pid, struct linux_proc_info* proc_info) {
    ...
    if (bytes_read == 0) {
        fclose(fp);
        free(buffer);
        error_and_exit("fread");
    }
    buffer[bytes_read] = '\0';  // Null-terminate the string
    fclose(fp);

    // There are 52 fields in /proc/[pid]/stat.
    // Note: The second field (tcomm) is enclosed in parentheses and may contain spaces.
    int ret = sscanf(buffer,
        ...
        &proc_info->it_real_value,
        &proc_info->start_time,
        &proc_info->vsize,
        &proc_info->rss,
        &proc_info->rsslim,
        &proc_info->start_code,
        &proc_info->end_code,
        ...
        &proc_info->policy,
        &proc_info->blkio_ticks,
        &proc_info->gtime,
        &proc_info->cgtime,
        &proc_info->start_data,
        &proc_info->end_data,
        &proc_info->start_brk,
        &proc_info->arg_start,
        &proc_info->arg_end,
